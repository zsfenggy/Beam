package me.stefan.beam.expansion.list;
...
public abstract class BeamListActivity<T extends BeamListActivityPresenter, M> extends BeamBaseActivity<T> {
    ...
    private void initAdapter(){
        ...
        if (mListConfig.mErrorAble){
            if (mListConfig.mErrorView != null) mAdapter.setMore(mListConfig.mErrorView,getPresenter());
            else  mAdapter.setError(mListConfig.mErrorRes);
            if (mListConfig.mErrorTouchToResumeAble)mListConfig.mErrorView.setOnClickListener(new View.OnClickListener() {
                @Override
